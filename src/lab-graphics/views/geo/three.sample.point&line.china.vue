...
let addGeoObject = function (d, group, svgObject) {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    for (let j = 0; j < simpleShapes.length; j++) {
      ...
      let points = simpleShape.getPoints();
      let spacedPoints = simpleShape.getSpacedPoints(100);
      ...
      particles.translateZ( - 1);
      particles.translateX( - center.x);
      particles.translateY( - center.y);
